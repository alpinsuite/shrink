...
//
// The drawing is the brand pack's `icons/shrink.svg`, restated in Dart because
...
//
//   container   rounded square, radius 23, gradient #3E9E80 -> #1E7059
//   glyph       white, stroke 6.5, round caps and joins
//               a large square at half opacity, a smaller one inside it,
//               and an arrow pulling the corner in
//
...
//   assets/icon/shrink.png                  512px, for the Linux desktop entry
//   packaging/icons/shrink-<size>.png       the hicolor sizes a .desktop needs
import 'dart:io';
...
/// The two stops of the brand's shrink gradient.
final img.Color _top = img.ColorRgba8(0x3E, 0x9E, 0x80, 0xFF);
final img.Color _bottom = img.ColorRgba8(0x1E, 0x70, 0x59, 0xFF);
...
img.Image draw(int size) {
  const supersample = 4;
  // Below about 24px the half-opacity element stops being a second plane and
  // becomes fog: it fills the gaps in the glyph and the whole thing reads as
  // one solid shape. The brand does the same thing to the house mark, which
  // drops to eight dots at larger radii below the same threshold rather than
  // shrinking nine into mush. A heavier stroke goes with it, because a 6.5-unit
  // line is under two pixels at 24 and disappears into the antialiasing.
  final small = size <= 24;
...
img.Image _drawAliased(int size, {required bool small}) {
  ...
  final weight = small ? 8.5 : _stroke;

  // The outgoing size: the large square, held back to half opacity because it
  // is what the image is leaving behind rather than what it becomes. Dropped
  // at small sizes, where it stops reading as a second plane.
  if (!small) {
    final ghost = layer(size);
    stroke(ghost, size, roundedRect(20, 20, 56, 56, 6), weight, _glyph);
    fade(ghost, 0.5);
    img.compositeImage(image, ghost);
  }

  // The incoming size, at full strength. Grown a little when it is on its own,
  // so the glyph still fills the tile once the large square is gone.
  stroke(
    image,
    size,
    small ? roundedRect(22, 38, 40, 40, 6) : roundedRect(20, 44, 32, 32, 5),
    weight,
    _glyph,
  );

  // The arrow, pulling the corner in: the diagonal, then the head.
  stroke(image, size, const <P>[P(72, 24), P(56, 40)], weight, _glyph);
  stroke(
    image,
    size,
    const <P>[P(72, 36), P(72, 24), P(60, 24)],
    weight,
    _glyph,
  );
...
Future<void> main() async {
  ...
  await write('assets/icon/shrink.png', img.encodePng(draw(512)));
  ...
  for (final size in <int>[16, 24, 32, 48, 64, 128, 256, 512]) {
    await write('packaging/icons/shrink-$size.png', img.encodePng(draw(size)));
  }
}

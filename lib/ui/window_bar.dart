...
///
/// Adapted from paint's `lib/ui/window_bar.dart`, which is where this shape was
/// worked out.
...
class _WindowBarState extends State<WindowBar> with WindowListener {
  ...
  Widget build(BuildContext context) {
    ...
    final title = queue.isEmpty
        ? l10n.appTitle
        : '${l10n.appTitle} — ${l10n.queueCount(queue.length)}';
...
/// The application's mark: a square with a smaller one inside it — the same
/// picture, smaller, which is the whole program.
///
...
class _MarkPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    // A 16-unit grid, the same one the kit's icons are drawn on.
    ...
    canvas
      ...
      // bounds instead of half a pixel outside them.
      ..drawRect(
        Rect.fromLTWH(1.9 * scale, 1.9 * scale, 12.2 * scale, 12.2 * scale),
        outline,
      )
      ..drawRect(
        Rect.fromLTWH(6.2 * scale, 6.2 * scale, 3.6 * scale, 3.6 * scale),
        fill,
      );
  }

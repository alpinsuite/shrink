#!/usr/bin/env bash
#
# Builds shrink_<version>_<arch>.deb from an already-built Flutter bundle.
#
...
# The bundle keeps its own layout under /usr/lib/shrink; only a launcher goes on
...
# tools/make_icons.dart writes packaging/icons/shrink-<n>.png; the hicolor theme
# wants <n>x<n>/apps/shrink.png, which is the name the desktop entry looks up.
...
gzip -9nc packaging/shrink.1 > "$STAGE/usr/share/man/man1/$PACKAGE.1.gz"
...
Homepage: https://buache.systems/shrink/
Description: batch image resizer
 Shrink resizes any number of images to a format, a longest edge, a quality
 and a file-size ceiling, each of which can be switched on independently.
 .
 Moving one control shows what the others would have to be to satisfy the
 whole set, so a size budget is met by choice rather than by trial. Everything
 happens on the local disk; the program opens no network connection.
CONTROL
